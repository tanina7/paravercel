...
function SeguimientoContent() {
  ...
  // Extraer el código de los parámetros de búsqueda de la URL
    ...
    async function fetchParamsAndData() {
      ...
      try {
        const [resTramite, resFirmas] = await Promise.all([
          fetch(`/api/verificar/${encodeURIComponent(rawCodigo)}`),
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-900 flex flex-col items-center justify-center text-white">
        ...
        <h2 className="text-xl font-bold">Buscando Registro...</h2>
    ...
    return (
      <div className="min-h-screen bg-gray-900 flex flex-col items-center justify-center p-4">
        <div className="bg-white rounded-2xl p-12 max-w-lg w-full text-center shadow-2xl">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Búsqueda Fallida</h2>
          <p className="text-gray-600 mb-8">{error}</p>
          <button onClick={() => router.push('/')} className="w-full bg-[#8B1A1A] text-white font-bold py-3 px-6 rounded-lg hover:bg-[#6b1414] transition-colors">
            Volver a intentar
          </button>
        </div>
      </div>
  ...
  // --- Caso donde el trámite NO está finalizado ---
  // Si no está "Listo para Impresion" o "Finalizado", mostramos una vista resumida del estado.
  // La API de verificación actual filtra por ('Listo para Impresion', 'Finalizado'),
  // así que si llega aquí, generalmente ya está finalizado.
  if (tramite.id_estado !== 7 && tramite.nombre_estado !== 'Listo para Impresion' && tramite.nombre_estado !== 'Finalizado') {
    return (
        <div className="min-h-screen bg-gray-50 flex flex-col items-center py-12 px-4 font-sans">
            <div className="w-full max-w-3xl flex justify-between items-center mb-8">
                <button onClick={() => router.push('/')} className="text-gray-500 hover:text-[#8B1A1A] font-medium flex items-center gap-2 transition-colors">
                <span>←</span> Volver al inicio
                </button>
                <div className="text-sm font-bold text-gray-400 uppercase tracking-widest">Estado del Trámite</div>
            </div>
            <div className="w-full max-w-3xl bg-white rounded-2xl shadow-xl border border-gray-100 p-8 text-center">
                <div className="text-5xl mb-4">⏳</div>
                <h2 className="text-2xl font-bold text-gray-900 mb-2">Trámite en Proceso</h2>
                <p className="text-gray-600 mb-6">El trámite <span className="font-bold">{tramite.tipo_tramite}</span> se encuentra en estado: <span className="text-[#8B1A1A] font-bold">{tramite.nombre_estado}</span>.</p>
                <p className="text-sm text-gray-500">Por favor, vuelve a consultar más adelante o revisa tu portal universitario.</p>
            </div>
        </div>
    )
  }

  // --- Caso Finalizado: Mostrar el certificado completo ---
  ...
  return (
    <div className="min-h-screen bg-[#1a1c23] py-12 px-4 flex flex-col items-center overflow-x-hidden font-sans">
      ...
      <div className="w-full max-w-[210mm] flex justify-between items-center mb-6">
        <button onClick={() => router.push('/')} className="text-gray-400 hover:text-white font-medium flex items-center gap-2 transition-colors text-sm">
      ...
      <div className="w-full max-w-[210mm] mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4 pb-12">
        <button 
